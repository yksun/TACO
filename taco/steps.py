...
def step_10_normalize(runner, embedded=False):
    ...
    # Compare-only FASTA — gets QC alongside assemblers but is excluded from
    # backbone selection, quickmerge, polishing, and purge_dups (see
    # taco.utils.EXCLUDED_FROM_BACKBONE).
    compare_fa = getattr(runner, "compare_fasta", None)
    if compare_fa and os.path.isfile(compare_fa) and os.path.getsize(compare_fa) > 0:
        shutil.copy(compare_fa, "./assemblies/compare.result.fasta")
        normalized += 1
...
def step_08_busco(runner):
    ...
    for fasta_path in assemblies:
        ...
        # Use the shared trial helper so all BUSCO call sites share the same
        # offline/online fallback, --download_path handling, and logging.
        result = _run_busco_trial(
            fasta_path, lineage, runner.threads, base, "busco",
            runner=runner)
        if result is None:
            runner.log_warn(f"BUSCO failed for {base}; checking BUSCO logs")
            busco_logs = (
                glob.glob(f"busco/{base}/logs/*.log")
                + glob.glob(f"busco/run_{base}/logs/*.log")
            )
...
def step_10_telomere_pool(runner):
    ...
    # Exclude leftover final assembly files from previous runs and any
    # user-supplied genomes (--reference, --compare).  These must not enter
    # the pool that feeds quickmerge / refinement.
    _exclude_prefixes = {"final", "final_merge", "backbone"} | set(EXCLUDED_FROM_BACKBONE)
...
def _run_merqury_for_assembly(runner, db, asm_fa, out_prefix, label):
    ...
    if qv or comp:
        ...
        if comp and not qv:
            found_qv = _merqury_metric_candidates_for_prefixes(prefixes, ".qv")
            found_msg = ", ".join(found_qv[:6]) if found_qv else "none"
            runner.log_warn(
                f"Merqury completeness was found for {label}, but no parseable "
                f"QV file was found near prefix '{out_prefix}'. Found QV "
                f"candidates: {found_msg}. Command log: {command_log}")
    else:
...
def _auto_select_backbone(runner):
    ...
    for idx, asm in enumerate(header[1:], start=1):
        ...
        if not asm:
            continue
        # User-supplied genomes (--reference, --compare) participate in QC
        # tables but must NEVER be selected as the refinement backbone.
        if asm.lower() in EXCLUDED_FROM_BACKBONE:
            continue
...
def _write_provenance_gff(final_fa, gff_path, name_map, protected_ids,
    ...
    lines.append(f"# Generated by TACO v1.4.0")
...
def _overhang_is_telomeric(seq, taxon="other", user_motif=None, min_score=0.08):
    """Return (is_telomeric, score) for a terminal-overhang sequence.

    Reuses TACO's own per-end scorer (telomere_detect.score_end) with the
    taxon's motif families, so "does this overhang carry a telomere" is decided
    the same way as detection everywhere else.  score_end scans both forward and
    reverse motif patterns, so orientation of the overhang does not matter.
    """
    if not seq or len(seq) < 12:
        return False, 0.0
    try:
        from taco.telomere_detect import (MOTIF_FAMILIES, get_taxon_families,
                                          score_end, revcomp)
        fam_keys = get_taxon_families(taxon) or list(MOTIF_FAMILIES.keys())
        family_patterns = {k: MOTIF_FAMILIES[k] for k in fam_keys
                           if k in MOTIF_FAMILIES}
        user_patterns = []
        if user_motif:
            m = re.sub(r"[^ACGTacgt]", "", str(user_motif)).upper()
            if len(m) >= 3:
                user_patterns = [("user:%s" % m,
                                  ("(%s){2,}" % m, "(%s){2,}" % revcomp(m)))]
        res = score_end(seq, len(seq), user_patterns, family_patterns)
        score = float(res.get("raw_score", 0.0))
...
def _parse_paf_rescue_hits(paf_path, donor_seqs, backbone_seqs, terminal_window=500,
                           taxon="other", user_motif=None):
    """Parse PAF for telomere rescue candidate screening with detailed metrics.

    In addition to alignment stats, this computes — per hit — the donor overhang
    that would extend a *touched backbone terminus* (strand-aware) and whether
    that overhang carries a telomere the backbone lacks.  This lets the screener
    accept short terminal extensions that add a telomere cap (often only a few
    hundred bp) instead of discarding them for falling under the generic
    ...
    with open(paf_path) as f:
        for ln in f:
            ...
            # Donor overhang that extends a touched backbone terminus. On '-'
            # strand the donor is reverse-complemented relative to the backbone,
            # so the prefix/suffix roles swap.
            donor_seq = donor_seqs.get(qname, "")
            term_ext = 0
            overhang_has_telomere = False
            overhang_telo_score = 0.0
            ends = []
            if touches_left:
                ends.append("left")
            if touches_right:
            ...
            for end in ends:
                if end == "left":
                    oh = donor_seq[:qs] if strand == "+" else donor_seq[qe:]
                else:
                    oh = donor_seq[qe:] if strand == "+" else donor_seq[:qs]
                term_ext = max(term_ext, len(oh))
                is_telo, sc = _overhang_is_telomeric(oh, taxon, user_motif)
                overhang_has_telomere = overhang_has_telomere or is_telo
                overhang_telo_score = max(overhang_telo_score, sc)
            ...
            hits.append({
                ...
                "ext": ext,
                "term_ext": term_ext,
                "overhang_has_telomere": overhang_has_telomere,
                "overhang_telo_score": round(overhang_telo_score, 4),
                "len_gain": len_gain,
...
def _screen_rescue_candidates(hits, min_ident=0.85, min_aligned_bp=8000,
    ...
    for h in best_per_pair.values():
        ...
        # A short extension is fine when the overhang adds a telomere the
        # backbone lacks — that terminal cap is exactly what rescue exists for.
        adds_telomere = h.get("overhang_has_telomere", False)
        if h["ext"] < min_ext and not adds_telomere:
        ...
        if reasons:
            ...
            score = (
                ...
                (2.0 if h.get("overhang_has_telomere") else 0.0) +
...
def _write_rescue_debug_tsv(hits, path):
    ...
    cols = ["donor", "backbone", "ident", "aligned_bp", "cov_backbone",
            "cov_donor", "ext", "term_ext", "overhang_telomere",
            "overhang_telo_score", "len_gain", "touches_left", "touches_right"]
    with open(path, "w", newline="") as f:
        w = csv.writer(f, delimiter="\t")
        w.writerow(cols)
        for h in hits:
            w.writerow([
                ...
                h["ext"], h.get("term_ext", ""),
                h.get("overhang_has_telomere", ""),
                h.get("overhang_telo_score", ""),
                h["len_gain"],
...
def _write_candidates_tsv(candidates, path):
    ...
    cols = ["candidate_rank", "source", "backbone", "donor",
            ...
            "cov_backbone", "cov_donor", "ext", "overhang_telomere", "len_gain",
    ...
    with open(path, "w", newline="") as f:
        ...
        for c in candidates:
            ...
            w.writerow([
                ...
                c.get("ext", ""), c.get("overhang_has_telomere", ""),
                c.get("len_gain", ""),
...
def _step12_busco_allow_download(runner=None):
    """Return True when BUSCO trials may fall back to online lineage lookup.

    Default is True (download is allowed) so a missing local cache does not
    silently abort BUSCO.  Users can opt out via:
      * --busco-offline-only on the CLI
      * STEP12_BUSCO_ALLOW_DOWNLOAD=0 in the environment (legacy)
    """
    if runner is not None and getattr(runner, "busco_offline_only", False):
        return False
    env = os.environ.get("STEP12_BUSCO_ALLOW_DOWNLOAD")
    if env is None:
        return True
    return env == "1"


def _busco_download_path(runner=None):
    """Return the BUSCO --download_path, or None to use BUSCO's default."""
    if runner is not None:
        path = getattr(runner, "busco_download_path", None)
        if path:
            return path
    return os.environ.get("BUSCO_DOWNLOAD_PATH") or None


def _busco_lineage_cached(lineage, runner=None):
    """Best-effort check: is the BUSCO lineage already on disk?

    Looks in --busco-download-path / BUSCO_DOWNLOAD_PATH (if set) and in the
    default BUSCO cache (./busco_downloads).  Returns True only when we can
    see the directory; absence does not guarantee the lineage is missing
    (BUSCO may be using a different cache).
    """
    if not lineage:
        return False
    candidates = []
    dl = _busco_download_path(runner)
    if dl:
        candidates.append(os.path.join(dl, "lineages", lineage))
        candidates.append(os.path.join(dl, lineage))
    candidates.append(os.path.join("busco_downloads", "lineages", lineage))
    candidates.append(os.path.join("busco", "busco_downloads", "lineages", lineage))
...
def _run_busco_trial(trial_fa, lineage, threads, trial_label, out_dir,
    ...
    # Common args (including --download_path when configured)
    download_path = _busco_download_path(runner)
    base_cmd = [
...
def _partial_t2t_thresholds(taxon):
    """Return minimum evidence for coverage-guided partial T2T replacement."""
    profiles = {
        "fungal": {
            "min_tcov": 0.15, "min_bp": 50000, "min_qcov": 0.80,
            "min_ident": 0.90,
        },
        "insect": {
            "min_tcov": 0.15, "min_bp": 50000, "min_qcov": 0.80,
        ...
        "plant": {
            "min_tcov": 0.50, "min_bp": 200000, "min_qcov": 0.85,
            "min_ident": 0.92,
        },
        "vertebrate": {
            "min_tcov": 0.50, "min_bp": 200000, "min_qcov": 0.85,
    ...
    return profiles.get(taxon, {
        "min_tcov": 0.30, "min_bp": 100000, "min_qcov": 0.80,
        "min_ident": 0.90,
    })
...
def _purge_dups_profile(taxon):
    ...
    profiles = {
        "fungal": {
            ...
            "note": "haploid-aware, upstream-default chaining",
        },
        "plant": {
            "mode": "single-round",
...
def _purge_dups_safety_check(runner, before_recs, after_recs, profile, mode,
    ...
    expected_improved = (
        expected_bp > 0
        and after_bp >= min_expected_bp
        and abs(after_bp - expected_bp) < abs(before_bp - expected_bp)
    )

    # Overshoot guard: purge_dups removes duplicated sequence, so it should move
    # size toward the expected genome size. If it instead pushes the assembly
    # FURTHER below expected (undershoot larger than the prior deviation) while
    # dropping a non-trivial fraction, it is likely removing unique, possibly
    # gene-bearing contigs rather than haplotigs — reject to preserve
    # completeness. Disable with PURGE_DUPS_ALLOW_OVERSHOOT=1.
    allow_overshoot = os.environ.get("PURGE_DUPS_ALLOW_OVERSHOOT", "0") == "1"
    undershoot_bp = max(0, expected_bp - after_bp) if expected_bp else 0
    prior_dev_bp = abs(before_bp - expected_bp) if expected_bp else 0
    overshoot_bad = (
        expected_bp > 0
        and undershoot_bp > prior_dev_bp
        and before_bp > 0
        and (before_bp - after_bp) / before_bp > 0.05
    )
    ...
    elif overshoot_bad and not allow_overshoot:
        verdict, reason = "reject", "overshoot_below_expected_size"
    elif before_bp > 0 and (before_bp - after_bp) / before_bp > max_drop \
            and not expected_improved:
        verdict, reason = "reject", "bp_drop_exceeds_taxon_limit"
    else:
        verdict = "accept"
        reason = "expected_size_improved" if expected_improved else "ok"
...
def step_12_refine(runner):
    ...
    v1.4.0 workflow — two-tier confidence model:
    ...
    """
    runner.log("Step 12 - Telomere-aware backbone refinement (v1.4.0)")
    os.makedirs("merqury", exist_ok=True)
    os.makedirs("assemblies", exist_ok=True)
    ...
    if not assembler:
        # Pick the first true assembler with a normalized result file.
        # User-supplied genomes (reference/compare) are excluded so they
        # never fall through into the refinement backbone.
        for asm in ALL_ASSEMBLERS:
            if asm in EXCLUDED_FROM_BACKBONE:
                continue
    ...
    if protected_fasta and os.path.isfile(protected_fasta) and os.path.getsize(protected_fasta) > 0:
        ...
        if shutil.which("minimap2") and len(protected_recs) > 0:
            ...
            asm_fastas = []
            for asm_name in [a for a in ALL_ASSEMBLERS if a not in EXCLUDED_FROM_BACKBONE]:
    ...
    # ====================================================================
    # BACKBONE-FIRST STRATEGY (v1.4.0)
    #
    ...
    if single_tel_src and shutil.which("minimap2"):
        ...
        all_hits = _parse_paf_rescue_hits(
            rescue_paf, stel_donor_seqs, backbone_seqs,
            taxon=getattr(runner, "taxon", "other"),
            user_motif=getattr(runner, "motif", None))
    ...
    max_novel_d_rise = float(os.environ.get("NOVEL_MAX_D_RISE",
                                             default_d_rise))
    partial_t2t = _partial_t2t_thresholds(taxon)
    partial_min_tcov = float(os.environ.get(
        "PARTIAL_T2T_MIN_TCOV", str(partial_t2t["min_tcov"])))
    partial_min_bp = int(os.environ.get(
        "PARTIAL_T2T_MIN_BP", str(partial_t2t["min_bp"])))
    partial_min_qcov = float(os.environ.get(
        "PARTIAL_T2T_MIN_QCOV", str(partial_t2t["min_qcov"])))
    partial_min_ident = float(os.environ.get(
        "PARTIAL_T2T_MIN_IDENT", str(partial_t2t["min_ident"])))
                ...
                if is_dup and best_target in current_backbone:
                    ...
                    if target_is_t2t:
                        ...
                        continue
                            ...
                            novel_len >= partial_min_bp and \
                            best_qcov >= partial_min_qcov and \
                            best_ident >= partial_min_ident:
                        # Partial overlap on Tier 2 with enough taxon-specific
                        # evidence to test whether the unmatched backbone
                        # sequence is real or low-coverage duplication.
                        ...
                        runner.log(
                            ...
                            f"tcov={best_tcov:.0%}, qcov={best_qcov:.0%}, "
                            f"id={best_ident:.1%}). Checking read coverage...")
                        ...
                        # Too small for a whole-contig upgrade and below the
                        # taxon-specific threshold for coverage-guided partial
                        # replacement.
                        runner.log(
                            f"  Novel T2T '{pname}' ({novel_len:,} bp) rejected: "
                            f"only covers {best_tcov:.0%} of Tier 2 "
                            f"'{best_target}' ({bb_len:,} bp), "
                            f"qcov={best_qcov:.0%}, id={best_ident:.1%}; "
                            f"partial replacement needs tcov≥{partial_min_tcov:.0%}, "
                            f"len≥{partial_min_bp:,} bp")
                        novel_rejected += 1
                        continue
    ...
    if novel_additions > 0 or novel_upgrades > 0 or novel_rejected > 0:
        runner.log(f"Novel pool T2T: {novel_additions} added, "
                   ...
                   f"{novel_rejected} rejected as duplicate/partial")
    ...
    if os.path.isfile(asm_fa) and os.path.isfile(final_fa):
        ...
        expected_size = _parse_genome_size(runner.genomesize)
        min_expected_ratio = float(os.environ.get(
            "PURGE_DUPS_MIN_EXPECTED_RATIO", "0.75"))
        size_moves_toward_expected = (
            expected_size > 0
            and fn_bp >= expected_size * min_expected_ratio
            and abs(fn_bp - expected_size) < abs(bb_bp - expected_size)
        )

        # Size sanity
        if bb_bp > 0 and fn_bp < bb_bp * 0.85 and \
                not size_moves_toward_expected:
        ...
        if warnings:
            ...
            with open(warn_file, "w") as f:
                f.write(f"TACO v1.4.0 — Refinement quality warnings\n")
...
def _final_busco_qc(runner):
    ...
    def _parse_busco_final():
        ...
        def from_counts(S, D, F, M, n):
            C = S + D
            return {
                "C": pct(C, n), "S": pct(S, n), "D": pct(D, n),
                "F": pct(F, n), "M": pct(M, n),
                "Ccnt": C, "Mcnt": M, "n": n,
            }

        def from_percentages(C, S, D, F, M, n):
            return {
                "C": float(C), "S": float(S), "D": float(D),
                "F": float(F), "M": float(M),
                "Ccnt": round(int(n) * float(C) / 100.0),
                "Mcnt": round(int(n) * float(M) / 100.0),
                "n": int(n),
            }
        ...
        for root in sorted(roots, key=os.path.getmtime, reverse=True):
            # Prefer full_table for exact counts.
            ...
            for jf in js_files:
                try:
                    ...
                    per = data.get("percentages") or {}
                    if n and per:
                        return from_percentages(
                            per.get("C", 0), per.get("S", 0), per.get("D", 0),
                            per.get("F", 0), per.get("M", 0), int(n))
                except Exception:
                    continue
    ...
    if metrics:
        rows = [
            ...
            ["BUSCO C (count)", str(metrics.get("Ccnt", ""))],
            ["BUSCO M (count)", str(metrics.get("Mcnt", ""))],
        ]
    else:
...
def _final_comparison_report(runner):
    ...
    qv_out = qv if qv else ("NA" if getattr(runner, 'merqury_enable', False) else "")
    comp_out = comp if comp else ("NA" if getattr(runner, 'merqury_enable', False) else "")
...
def _compare_vs_final_report(runner):
    ...
    Produces ``final_results/compare_report/`` containing:

      * ``compare_vs_final.paf``  — minimap2 ``-cx asm5`` alignment of the
        compare FASTA against TACO's ``final.merged.fasta``.
      * ``contig_to_contig.tsv`` — per-compare-contig 1-to-1 best hit on
        the final assembly: target contig, query/target lengths, aligned
        bases, average identity, query/target coverage.
      * ``weak_regions.tsv``     — windows of ``final.merged.fasta`` that
        have no/low coverage from the compare assembly.  Useful for
        spotting regions where TACO either has extra sequence or where the
        compare assembly is missing sequence.
      * ``compare_quast/``        — QUAST run with ``-R compare.fa`` on the
        final assembly (when QUAST is available), giving genome-fraction,
        misassembly, and NA50 metrics relative to the compare genome.
      * ``compare_dnadiff/``      — optional MUMmer ``dnadiff`` summary
        when ``dnadiff`` is on PATH (skipped silently otherwise).

    The report is read-only: nothing here modifies ``final.merged.fasta``.
    """
    compare_fa = getattr(runner, "compare_fasta", None)
    ...
    if not shutil.which("minimap2"):
        runner.log_warn(
            "Compare report skipped: minimap2 not found on PATH.")
        return

    out_dir = "final_results/compare_report"
    os.makedirs(out_dir, exist_ok=True)
    ...
    with open(paf_path) as f:
        for line in f:
            ...
            if len(cols) < 12:
                continue
            qname, qlen, qs, qe, strand, tname, tlen, ts, te, matches, alen = (
    ...
    quast_bin = shutil.which("quast.py") or shutil.which("quast")
    if quast_bin:
        quast_dir = os.path.join(out_dir, "compare_quast")
        cmd = (f"{quast_bin} {final_fa} -R {compare_fa} "
               f"--threads {runner.threads} -o {quast_dir} "
               f"--labels final_vs_compare")
        runner.run_cmd(cmd, desc="QUAST: final.merged vs --compare",
                       check=False)
...
# ── v1.4.0: purification before measurement ──────────────────────────────────
#
...
def step_13_final_qc(runner):
    ...
    # The compare-vs-final contig-to-contig report runs in step 14 (sub-step
    # 14C) so that resumed runs invoking only -s 14 --compare X still get
...
def step_14_report(runner):
    ...
    Sub-modes selected automatically:
      14A (full mode): builds final comparison table from Step 13 QC metrics
           + per-assembler metrics from Step 10.
      14B (assembly-only): builds assembly-only comparison table from Step 10
           metrics, copies to ``final_results/``.
      14C (compare add-on, runs whenever ``--compare`` is set): emits the
           contig-to-contig comparison report and weak_regions analysis of
           ``final.merged.fasta`` against the user-supplied ``--compare``
           FASTA into ``final_results/compare_report/``.  Runs *before*
           cleanup so the artefacts land in their final location and aren't
           wiped by the move-into-temp pass.
    """
    if runner.assembly_only:
        step_14b_assembly_only_report(runner)
        # Compare-vs-final report makes no sense in assembly-only mode (no
        # final.merged.fasta), so 14C is skipped here by design.
    else:
